...
if websocket_status then
    ...
    function handler(req)
        local req = req or ""
        local req = string.lower(tostring(req))
        local input = rconsoleinput()
    
        client.utility.output_console("print", "Sending Request...")
        local s,e = main_websocket:Send(input)
    
        if s then
            client.utility.output_console("error", e)
        else
            client.utility.output_console("success", "Request Sent!")
        end

        handler(req)
    end

    client.utility.output_console("print", "Send a request!")
